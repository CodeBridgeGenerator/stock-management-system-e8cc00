...
import orderItemsFakerFactory from "./orderItemsFakerFactory";
...
const OrderItemsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = orderItemsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("orderItems").create(elem));
...
export default connect(mapState, mapDispatch)(OrderItemsFakerDialogComponent);
