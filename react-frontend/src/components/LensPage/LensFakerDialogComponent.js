...
import lensFakerFactory from "./lensFakerFactory";
...
const LensFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [brandidIds, setbrandidIds] = useState();
...
client.service("categories")
    ...
    const onRun = async () => {
        let fakeData = lensFakerFactory(props.user,fakerCount,brandidIds,categoryidIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("lens").create(elem));
...
export default connect(mapState, mapDispatch)(LensFakerDialogComponent);
