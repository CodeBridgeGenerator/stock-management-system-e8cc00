...
const LensDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    const dt = useRef(null);
    const urlParams = useParams();
    const [showUpload, setShowUpload] =useState(false)

const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.lensname}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.price}</p>
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.availablestockquantity}</p>
const dropdownTemplate3 = (rowData, { rowIndex }) => <p >{rowData.brandid?.name}</p>
const dropdownTemplate4 = (rowData, { rowIndex }) => <p >{rowData.categoryid?.categoryname}</p>
...
<Column field="lensname" header="Lens Name" body={pTemplate0} sortable style={{ minWidth: "8rem" }} />
<Column field="price" header="Price" body={pTemplate1} sortable style={{ minWidth: "8rem" }} />
<Column field="availablestockquantity" header="Available Stock Quantity" body={pTemplate2} sortable style={{ minWidth: "8rem" }} />
<Column field="brandid" header="Brand ID" body={dropdownTemplate3} style={{ minWidth: "8rem" }} />
<Column field="categoryid" header="Category ID" body={dropdownTemplate4} style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Lens Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>
        </>
    );
