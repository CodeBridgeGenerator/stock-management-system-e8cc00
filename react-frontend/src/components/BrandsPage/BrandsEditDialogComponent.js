...
const BrandsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        const result = await client.service("brands").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info brands updated successfully" });
    ...
    return (
        <Dialog header="Edit undefined" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            style={{ maxWidth: "55vw" }} role="brands-edit-dialog-component">
...
export default connect(mapState, mapDispatch)(BrandsCreateDialogComponent);
