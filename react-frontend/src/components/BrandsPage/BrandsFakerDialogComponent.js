...
import brandsFakerFactory from "./brandsFakerFactory";
...
const BrandsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = brandsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("brands").create(elem));
...
export default connect(mapState, mapDispatch)(BrandsFakerDialogComponent);
