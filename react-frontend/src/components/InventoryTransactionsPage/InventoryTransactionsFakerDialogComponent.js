...
import inventoryTransactionsFakerFactory from "./inventoryTransactionsFakerFactory";
...
const InventoryTransactionsFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("lens")
                        .find({ query: { $select: ["_id","lensname"] } })
                        .then((data) => {
                        setlensIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = inventoryTransactionsFakerFactory(props.user,fakerCount,lensIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("inventoryTransactions").create(elem));
...
export default connect(mapState, mapDispatch)(InventoryTransactionsFakerDialogComponent);
