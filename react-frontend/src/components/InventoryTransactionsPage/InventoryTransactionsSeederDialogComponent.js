...
import seederData from "./inventoryTransactions.seeder.json";
...
const InventoryTransactionsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("inventoryTransactions").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(InventoryTransactionsSeederDialogComponent);
