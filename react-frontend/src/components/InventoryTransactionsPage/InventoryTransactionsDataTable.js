...
const InventoryTransactionsDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    const dt = useRef(null);
    const urlParams = useParams();
    const [showUpload, setShowUpload] =useState(false)

const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.transactionDate}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.quantityChange}</p>
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.transactionType}</p>
const dropdownTemplate3 = (rowData, { rowIndex }) => <p >{rowData.lens?.lensname}</p>
...
<Column field="transactionDate" header="Transaction Date" body={pTemplate0} sortable style={{ minWidth: "8rem" }} />
<Column field="quantityChange" header="Quantity Change" body={pTemplate1} sortable style={{ minWidth: "8rem" }} />
<Column field="transactionType" header="Transaction Type" body={pTemplate2} sortable style={{ minWidth: "8rem" }} />
<Column field="lens" header="Lens" body={dropdownTemplate3} style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload InventoryTransactions Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>
        </>
    );
