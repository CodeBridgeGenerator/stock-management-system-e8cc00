...
const LensesDetailsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            lenspower: _entity?.lenspower,
        };

        setLoading(true);
        try {
            
        const result = await client.service("lensesDetails").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info lensesDetails updated successfully" });
    ...
    return (
        <Dialog header="Edit undefined" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            style={{ maxWidth: "55vw" }} role="lensesDetails-edit-dialog-component">
                <div className="col-12 md:col-6 field mt-5">
        <span className="align-items-center">
            <label htmlFor="lenspower">Lens Power:</label>
            <InputText id="lenspower" className="w-full mb-3 p-inputtext-sm" value={_entity?.lenspower} onChange={(e) => setValByKey("lenspower", e.target.value)}  />
        </span>
        </div>
...
export default connect(mapState, mapDispatch)(LensesDetailsCreateDialogComponent);
