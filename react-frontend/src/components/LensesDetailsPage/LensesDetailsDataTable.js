...
const LensesDetailsDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    const dt = useRef(null);
    const urlParams = useParams();
    const [showUpload, setShowUpload] =useState(false)

const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.lenspower}</p>
...
<Column field="lenspower" header="Lens Power" body={pTemplate0} sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload LensesDetails Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>
        </>
    );
...
export default LensesDetailsDataTable;