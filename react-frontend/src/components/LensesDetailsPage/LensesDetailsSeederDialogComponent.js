...
import seederData from "./lensesDetails.seeder.json";
...
const LensesDetailsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("lensesDetails").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(LensesDetailsSeederDialogComponent);
