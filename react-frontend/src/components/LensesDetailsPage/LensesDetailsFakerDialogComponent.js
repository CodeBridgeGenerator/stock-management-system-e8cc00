...
import lensesDetailsFakerFactory from "./lensesDetailsFakerFactory";
...
const LensesDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = lensesDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("lensesDetails").create(elem));
...
export default connect(mapState, mapDispatch)(LensesDetailsFakerDialogComponent);
